...
def AddNew():
    ...
    cursor.execute("INSERT INTO person (name, email, startup, plano, online_offline, senha_numerica, tag, telefone, rg, cpf) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",(str(NAME.get()), str(EMAIL.get()), str(STARTUP.get()), str(PLANO.get()),str(ONLINE_OFFLINE.get()),str(SENHA_NUMERICA.get().upper()),str(TAG.get()), int(TELEFONE.get()), int(RG.get()), int(CPF.get())))
    ...
    messagebox.showinfo("Cadastro", "O Usuario foi cadastrado")
#################################################################################################
